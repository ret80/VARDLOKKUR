  ...
  private buildMapTextures(map: WorldData) {
    ...
    for (let y = 0; y < H; y++) for (let x = 0; x < W; x++) {
      ...
      switch (t) {
        ...
        case Tl.HOUSE: gx.fillStyle = "#2c2620"; gx.fillRect(X, Y, T, T); break;
  ...
  private paintWall(ctx: CanvasRenderingContext2D, t: number, v: number, dungeonId: number) {
    ...
    switch (t) {
      ...
      case Tl.PALISADE:
        ...
        break;
      case Tl.HOUSE: {
        // Скандинавский длинный дом: массивные стены из тёмных брёвен,
        // высокая двускатная крыша с дерном, резной конёк-дракон.
        // Рисуем один большой дом на всю клетку 16x16
        
        // Фундамент/основание - каменная основа со мхом
        P(0, 12, 16, 4, 0x3a3530);
        P(0, 14, 16, 2, 0x4a4540);
        
        // Стены - горизонтальные брёвна с текстурой
        for (let i = 0; i < 7; i++) {
          const y = 4 + i * 2;
          const logColor = i % 2 === 0 ? 0x4a3520 : 0x3d2c18;
          P(0, y, 16, 2, logColor);
        }
        
        // Вертикальные балки по краям и в центре
        P(0, 4, 2, 10, 0x2a1f10);
        P(14, 4, 2, 10, 0x2a1f10);
        P(7, 4, 2, 10, 0x2a1f10);
        
        // Окна с ставнями (два окна)
        P(2, 8, 4, 4, 0x1a1208);
        P(10, 8, 4, 4, 0x1a1208);
        // Свет из окон
        P(3, 9, 2, 2, v & 1 ? 0xffcc66 : 0x667788);
        P(11, 9, 2, 2, v & 1 ? 0xffcc66 : 0x667788);
        
        // Дверь - большая арочная с резьбой
        P(5, 8, 6, 8, 0x2a1f10);
        P(6, 7, 4, 1, 0x2a1f10); // арка над дверью
        P(7, 10, 2, 6, 0x1a1208); // проём двери
        
        // Крыша - высокая двускатная с дерном
        // Нижний скат
        P(-2, 2, 20, 3, 0x5a4a38);
        P(-1, 0, 18, 3, 0x4a3e2c);
        // Средний скат
        ...
        // Дерн на крыше с пятнами снега
        P(0, 2, 16, 1, 0x4d5f44);
        P(2, 0, 12, 1, 0x41503a);
        P(4, -4, 8, 1, 0x3d4a35);
        
        // Снег на коньке и краях крыши
        P(6, -16, 4, 1, 0xc8d3dc);
        P(-2, 3, 3, 1, 0xc8d3dc);
        P(15, 3, 3, 1, 0xc8d3dc);
        ...
        // Дым из трубы (если есть вариация)
        if (v & 1) {
          P(12, -2, 2, 3, 0x666666);
          P(11, -4, 3, 2, 0x555555);
        }
