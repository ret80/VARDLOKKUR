...
export class CrawlerRenderer extends BaseEnemyRenderer {
  protected drawBody(g: GraphicsHandle, data: IEnemyData, ctx: RenderContext): void {
    ...
    if (e.hidden) {
      px(r, g, -3, -1, 6, 2, 0x3a3226, 0.7);
      return;
    }
    const rt = ctx.tint as (c: number) => number;
    const alpha = ctx.a as number;
    const wig = Math.sin(e.t * 12) * 1.5;
    px(r, g, -5, -2 + bob, 10, 5, rt(0x4a5238), alpha);
    px(r, g, -5, -2 + bob, 10, 2, rt(0x5a6244), alpha);
    px(r, g, -6 + wig, 0 + bob, 2, 3, 0x3a4228, alpha);
    px(r, g, 4 - wig, 0 + bob, 2, 3, 0x3a4228, alpha);
    px(r, g, fx * 3, -1 + bob, 2, 1, 0xe0a030, alpha);
  }
}
