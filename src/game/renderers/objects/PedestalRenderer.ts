...
export class PedestalRenderer implements Renderer<IPedestalData> {
  render(g: GraphicsHandle, data: IPedestalData, ctx: RenderContext): void {
    ...
    clearGraphics(r, g);
    const time = ctx.time;
    ell(r, g, 0, 7, 8, 2.6, 0x05080d, 0.5);
    px(r, g, -6, 2, 12, 4, 0x4e5a68);
    px(r, g, -6, 2, 12, 1, 0x5c6875);
    px(r, g, -4, -6, 8, 8, 0x39424e);
    px(r, g, -4, -6, 8, 1, 0x4e5a68);
    px(r, g, -2, -12, 4, 6, 0x4e5a68);
    if (!data.taken) {
      ...
      px(r, g, -1, -11, 2, 4, col, pulse);
      px(r, g, -1, -10, 1, 2, core, pulse);
      const cr = ((col >> 16) & 0xff) / 255;
      const cg = ((col >> 8) & 0xff) / 255;
      const cb = (col & 0xff) / 255;
      ell(r, g, 0, -9, 6, 6, col, pulse * 0.6);
    } else {
      px(r, g, -1, -11, 2, 4, 0x232c38);
    }
  }
}
