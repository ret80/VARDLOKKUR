...
export class CombatSystem {
  ...
  private get projectiles() { return this.store.entities.projectiles; }
  private get map() { return this.store.map!; }
  private get doors() { return this.store.entities.doors; }
  ...
  trySword() {
    const p = this.player;
    const f = this.flags;
    if (!f.hasItem("sword")) { this.float(p, "Нужен клинок", 0x6e7f8d); return; }
    if (p.swingT > 0) return;
    this.playerDomain?.resetTimers();
    audio.swing();
    if (this.store.playerDomain) this.store.playerDomain.swingT = 0.22;
    const a = Math.atan2(p.dir.y, p.dir.x);
    const dmg = f.hasEnhancement("swordUp") ? 2 : 1;
    for (const e of this.enemies.all) {
      ...
      if (f.hasItem("hammer") && !e.dead && e.freezeT <= 0) { e.freezeT = 0.8; }
    }
  }
  ...
  tryAxe() {
    const p = this.player;
    const f = this.flags;
    if (!f.hasItem("axe")) { this.float(p, "Секира у Жнеца", 0x6e7f8d); return; }
    if (this.axeState !== "ready") { audio.locked(); return; }
    this.axeState = "out";
    const a = Math.atan2(p.dir.y, p.dir.x);
    const dmg = f.hasEnhancement("axeUp") ? 2 : 1;
  ...
  damagePlayer(dmg: number, sx: number, sy: number, pierce = false) {
    ...
    p.hp = this.playerDomain!.takeDamage(dmg, sx, sy);
    audio.hurt();
  ...
  updateProjectiles(dt: number) {
    const p = this.player;
    const m = this.map;
    for (let i = this.projectiles.all.length - 1; i >= 0; i--) {
      const pr = this.projectiles.all[i] as ProjectileRt;
      pr.life -= dt;
  ...
  onDungeonBossDeath(payload: { id: number }) {
    const m = this.map;
    const f = this.flags;
    if (m.dungeonId === 0) f.setFlag("reaperDead", true);
    else if (m.dungeonId === 1) f.setFlag("spiderDead", true);
    else f.setFlag("giantDead", true);
    const boss = this.store.bossRef;
  ...
  onSnakeDeath() {
    this.flags.setFlag("snakeDead", true);
    this.store.bossRef = null;
    audio.bossDie();
  ...
  startDungeonBoss() {
    ...
    const e = this.services.spawnEnemy(kind, m.bossSpot.x, m.bossSpot.y);
    e.state = "enter"; e.stateT = 1.0;
    if (m.dungeonId === 0) e.seed = 1;
    this.store.bossRef = e;
    audio.horn();
  ...
  startSnakeBattle() {
    ...
    f.setFlag("snakeStarted", true);
    const m = this.map;
    const e = this.services.spawnEnemy("snake", m.snakeSpot.x, m.snakeSpot.y - 10);
