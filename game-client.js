      ...
      try {
        ...
        if (data.type === 'command') {
          ...
          try {
            switch (cmd.type) {
              case 'teleport':
                result = s.teleportPlayer ? s.teleportPlayer(cmd.x, cmd.y) : null;
                console.log('[game-client] teleport result:', result);
                break;
              case 'set-hp':
                result = s.setPlayerHp ? s.setPlayerHp(cmd.hp) : null;
              ...
              case 'full-heal-player':
                ...
                break;
              case 'spawn-enemy':
                result = s.spawnEnemy ? s.spawnEnemy(cmd.kind, cmd.x, cmd.y) : null;
                console.log('[game-client] spawn-enemy result:', result);
                break;
              case 'remove-enemy':
                result = s.removeEnemy ? s.removeEnemy(cmd.eid) : null;
              ...
              case 'remove-projectile':
                result = s.removeProjectile ? s.removeProjectile(cmd.eid) : null;
                break;
              case 'clear-projectiles':
                result = s.clearProjectiles ? s.clearProjectiles() : null;
                break;
              case 'remove-drop':
                result = s.removeDrop ? s.removeDrop(cmd.eid) : null;
                break;
              case 'clear-drops':
                result = s.clearDrops ? s.clearDrops() : null;
                break;
              case 'set-timescale':
                s.setTimeScale && s.setTimeScale(cmd.scale);
                result = true;
                break;
              case 'freeze':
              ...
              case 'unfreeze':
                ...
                break;
              case 'add-runes':
                s.addRunes && s.addRunes(cmd.count || 1);
                result = true;
                break;
              case 'add-arrows':
                s.addArrows && s.addArrows(cmd.count || 12);
                result = true;
                break;
              case 'add-hearts':
                s.addHearts && s.addHearts(cmd.count || 1);
                result = true;
                break;
              case 'set-flag':
                s.setFlag && s.setFlag(cmd.key, cmd.value);
                result = true;
                break;
              default:
                console.warn('[game-client] Unknown command:', cmd.type);
            }
          } catch (e) {
